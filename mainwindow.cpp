    ...
    ui(new Ui::MainWindow)
{
    ...
    */
    ...
    //Small exemple configuration, uncomment everything above for the real life exemple (and discomment two following lines)
    ...
    // Setting up parameters for computation of power received at RX, see in syllabus
...
bool isNotBlocked(QPointF A, QPointF B, QList<Wall> walls){
    ...
    for(int i=0; i<walls.size(); i++){
        if( walls[i].intersect( QLineF(A,B), nullptr)==1 && isNotOnWall(A,walls[i]) && isNotOnWall(B,walls[i])){
            return false;
        }
    }
